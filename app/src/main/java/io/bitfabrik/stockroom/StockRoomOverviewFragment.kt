...
import io.bitfabrik.stockroom.databinding.DialogAddRemoveSelectionBinding
import io.bitfabrik.stockroom.databinding.FragmentOverviewBinding
...
import io.bitfabrik.stockroom.utils.getAssetsCapitalGain
import io.bitfabrik.stockroom.utils.getCapitalGainLossText
            ...
            builder.setView(dialogBinding.root)
                ...
                { _, _ ->
                    ...
                    overviewSelection.addAll(
                        sharedPreferences.getStringSet("overview_selection", emptySet())
                            ?: emptySet()
                    )
...
//    }
        ...
        if (stockitemList.size > 3) {
            ...
            val activeStockAssets = stockitemList.filter { item ->
                val (quantity, price, fee) = getAssets(item.assets)
                quantity > 0.0
            }

            summaryStr.append("${requireContext().getString(R.string.summary_stocks_with_active_assets)} ")
