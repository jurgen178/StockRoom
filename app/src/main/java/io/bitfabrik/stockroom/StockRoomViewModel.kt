...
import io.bitfabrik.stockroom.database.Events
import io.bitfabrik.stockroom.database.Group
import io.bitfabrik.stockroom.database.StockDBdata
import io.bitfabrik.stockroom.database.StockRoomDatabase
import io.bitfabrik.stockroom.database.StoreData
import io.bitfabrik.stockroom.list.DebugData
import io.bitfabrik.stockroom.utils.getAssets
import io.bitfabrik.stockroom.utils.getGroupsMenuList
import io.bitfabrik.stockroom.utils.isOnline
import io.bitfabrik.stockroom.utils.isValidSymbol
import io.bitfabrik.stockroom.utils.saveTextToFile
...
class StockRoomViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private val stockMarketDataRepository: StockMarketDataRepository =
            StockMarketDataRepository(
                    ...
                    { StockMarketDataOkxApiFactory.marketDataApi }
            )
        ...
        for (i in 0 until size) {
            ...
            if (isValidSymbol(symbol)) {
                ...
                if (jsonObj.has("type")) {
                    ...
                    // -1 is the last data provider 'None', to allow adding new data provider types before.
                    if (type == -1) {
                        type = DataProvider.None.value
                    }
                }
    ...
    fun exportJSON(
        ...
        val stockItemsJson = stockItemList.map { stockItem ->
            ...
            val typeValue = if (stockItem.stockDBdata.type != 0) {
                // -1 is the last data provider 'None', to allow adding new data provider types before.
                if (stockItem.stockDBdata.type == DataProvider.None.value) {
                    -1
                } else {
